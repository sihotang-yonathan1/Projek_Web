    ...
    function GET(){
        ...
        if ($max_items_number != -1){
            $prepared_statement = $this->connection->prepare("
                SELECT 
                    id, nama, tanggal, waktu, jumlah_tiket, jenis_tiket, catatan_khusus, status 
                FROM reservasi_form 
                LIMIT ?
            ");
    ...
    function POST(){
        ...
        $prepared_statement = $this->connection->prepare("
            INSERT INTO reservasi_form 
                (nama, tanggal, waktu, jumlah_tiket, jenis_tiket, catatan_khusus, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
    
        $prepared_statement->bind_param(
            "sssisss", 
            $req['nama'], $req['tanggal'], $req['waktu'], $req['jumlah_tiket'], $req['jenis_tiket'], $catatan_khusus, $status
        );
